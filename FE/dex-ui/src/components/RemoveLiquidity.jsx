...
import { getRouter, getFactory, getPair, fetchPairAddress, fetchReserves } from "../services/dexApi.js";
 ...
 */
export default function RemoveLiquidity({ tokens, provider, signerPromise, address, onSuccess }) {
  ...
  const fetchPoolData = useCallback(async () => {
    ...
    try {
      ...
      const pairAddress = await fetchPairAddress(factory, tokenA.address, tokenB.address);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <select
            ...
            className="w-full px-3 py-2 rounded-xl bg-slate-800 text-white outline-none border border-slate-700"
          >
        ...
        <div className="rounded-xl p-4 bg-slate-800/40 text-sm">
          ...
          <div className="flex justify-between text-slate-400">
            ...
            <span className="text-indigo-400">
              {((Number(lpBalance) / Number(poolInfo.totalSupply)) * 100).toFixed(2)}%
            </span>
          </div>
        </div>
      )}
          ...
          <div className="space-y-3">
            <div className="rounded-xl p-4 bg-slate-800/40 border border-slate-700">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xl font-semibold text-white">{amountsOut.amountA}</span>
              </div>
            </div>
            <div className="rounded-xl p-4 bg-slate-800/40 border border-slate-700">
              <div className="flex items-center justify-between">
                <span className="text-slate-400">{tokenB?.symbol}</span>
                <span className="text-xl font-semibold text-white">{amountsOut.amountB}</span>
              </div>
            </div>
          </div>
        </>
      ...
      {!poolInfo?.exists && tokenA && tokenB && (
        <div className="p-3 rounded-xl bg-yellow-500/10 border border-yellow-500/20 flex items-start gap-2">
          ...
          <span className="text-sm text-yellow-400">Pool does not exist for this pair</span>
        </div>
      )}
      ...
      {lpBalance === "0" && poolInfo?.exists && (
        <div className="p-3 rounded-xl bg-yellow-500/10 border border-yellow-500/20 flex items-start gap-2">
          <Info className="w-4 h-4 text-yellow-400 mt-0.5" />
          <span className="text-sm text-yellow-400">You don't have any liquidity in this pool</span>
        </div>
      )}
